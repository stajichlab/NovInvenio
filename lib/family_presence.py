...
minimum, both from that SAME target (never mixing one target's E-value with another's
coverage, and never pooling coverage across targets -- see parse_domtblout).
...
def parse_domtblout(path, default_evalue, min_coverage):
    ...
    HMM length, on that target) >= min_coverage -- never mixing one target's E-value
    with a different target's coverage, and never pooling coverage across different
    targets (either would let a family look present on evidence no single target
    actually supports). A query is present in this proteome if ANY target qualifies
    (mirrors bin/profile_to_matrix.py's semantics for the general mmseqs pathway) --
    querying only the single best-E target's own coverage would wrongly call a family
    absent when its best-E hit happens to be a fragment but a different, slightly
    weaker-E target has full coverage (2026-09-03 review finding). A query with no
    qualifying target is omitted from the result.
    ...
    for (query, _target), (qlen, full_e, intervals) in per_target.items():
        coverage = _merged_span(intervals) / qlen if qlen > 0 else 0.0
        if not (full_e < default_evalue and coverage >= min_coverage):
            continue
...
def family_presence_by_proteome(domtblout_paths, default_evalue, min_coverage,
    ...
    per-family calibrated) and the minimum profile coverage -- see parse_domtblout.
    """
    ...
    for dom_path in domtblout_paths:
        ...
        for suffix in domtblout_suffixes:
            if short.endswith(suffix):
                ...
                break
        for query in parse_domtblout(dom_path, default_evalue, min_coverage):
