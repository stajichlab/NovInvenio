...
# Version comes from pixi.toml's `[workspace] version = "X.Y.Z"` -- the one
# place this repo already records it (see CLAUDE.md's pixi.toml convention) --
# rather than a second hardcoded copy here that would drift from a release.
NOVINVENIO_REPO_URL = "https://github.com/stajichlab/NovInvenio"
_REPO_ROOT = Path(__file__).resolve().parent.parent
_PIXI_TOML = _REPO_ROOT / "pixi.toml"


def _read_novinvenio_version() -> str:
    try:
        text = _PIXI_TOML.read_text()
    except OSError:
        return "unknown"
...
# The fixed/tagged version -- pixi.toml's own `version`, bumped by hand at
# release time. This alone can't distinguish a released build from an active
# checkout that has moved on since that bump (see NOVINVENIO_DISPLAY_VERSION).
NOVINVENIO_VERSION = _read_novinvenio_version()

_DESCRIBE_RE = re.compile(r'^v?(?P<tag>.+)-(?P<dist>\d+)-g(?P<hash>[0-9a-f]+)(?P<dirty>-dirty)?$')


def _git_describe() -> str | None:
    """``git describe --tags --always --long --dirty`` against the repo this
    file lives in, or None when that isn't possible (no git binary, no .git --
    e.g. a Docker image or tarball checkout that ships bin/lib without
    history, a bare pixi env, etc). Never raises -- version display always
    has a fallback."""
    try:
        proc = subprocess.run(
            ['git', '-C', str(_REPO_ROOT), 'describe', '--tags', '--always', '--long', '--dirty'],
...
def _build_display_version() -> tuple[str, str]:
    """Combine the fixed pixi.toml version with the active git commit.

    Returns ``(display_version, status)`` where ``status`` is one of
    ``'release'`` (HEAD is exactly the tagged commit, clean tree --
    NOVINVENIO_VERSION is exactly what's running), ``'dev'`` (HEAD has moved
    past that tag, or the tree is dirty -- an active/in-development build,
    identified by its short commit hash so two dev builds are distinguishable
    even though they share the same fixed version number), or ``'unknown'``
    (no git info available at all, e.g. a container/tarball build -- only the
    fixed version can be shown).
    """
    fixed = NOVINVENIO_VERSION
    described = _git_describe()
    if described is None:
        return fixed, 'unknown'
    m = _DESCRIBE_RE.match(described)
    if m is None:
        # No tags reachable in this history at all -- just a bare hash
        # (--always's fallback), optionally dirty.
        dirty = described.endswith('-dirty')
        short_hash = described[:-len('-dirty')] if dirty else described
        suffix = f'+g{short_hash}' + ('.dirty' if dirty else '')
        return f'{fixed}{suffix}', 'dev'
    dist = int(m.group('dist'))
    dirty = m.group('dirty') is not None
    if dist == 0 and not dirty:
        return fixed, 'release'
    suffix = f'+g{m.group("hash")}' + ('.dirty' if dirty else '')
    return f'{fixed}{suffix}', 'dev'


# NOVINVENIO_DISPLAY_VERSION is what every generated page actually shows --
# NOVINVENIO_VERSION plus a `+g<hash>` build tag (PEP 440 local-version style)
# whenever the checkout that produced the page isn't exactly the tagged
# release (ahead of the tag and/or an uncommitted working-tree change).
# NOVINVENIO_RELEASE_STATUS labels which case applied, so a report can be told
# apart from "the v0.5.0 tag" vs. "some commit after v0.5.0" at a glance.
NOVINVENIO_DISPLAY_VERSION, NOVINVENIO_RELEASE_STATUS = _build_display_version()

_STATUS_LABEL = {'release': ' (release)', 'dev': ' (dev)', 'unknown': ''}

# Shared credit footer for every generated page (the four report/landing
# pages) -- repo url and version are fixed at generation time, not user data,
# so this is inserted as-is rather than through the textContent-only path
# used for FASTA-header-derived strings elsewhere on these pages.
FOOTER_CSS = r"""
  .ni-footer {
    margin-top: 28px; padding-top: 12px; border-top: 1px solid var(--border);
    color: var(--muted); font-size: 11px;
  }
  .ni-footer a { color: var(--text-secondary); }
"""

FOOTER_HTML = (
    '<footer class="ni-footer">Generated by '
    f'<a href="{NOVINVENIO_REPO_URL}">NovInvenio</a> v{NOVINVENIO_DISPLAY_VERSION}'
    f'{_STATUS_LABEL[NOVINVENIO_RELEASE_STATUS]}'
    '</footer>'
)
