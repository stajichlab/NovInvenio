...
# A real hmmscan --tblout line (target = Pfam-A HMM, query = candidate protein --
# reversed from hmmsearch). Column positions that matter to the parser:
#   parts[0] domain name, parts[1] accession, parts[2] protein/query id, parts[4] E-value
TBLOUT = """\
...
Protein_kinase       PF00069.28  candidate1           -           1.2e-30  105.3   0.1   3.4e-31  104.9   0.1     1      260      10      265       8      270  0.95 protein kinase domain
Protein_kinase       PF00069.28  candidate2           -           2.0e-25   90.1   0.2   5.1e-26   89.7   0.2     1      258      12      260      10      263  0.93 protein kinase domain
zf-C2H2               PF00096.28  candidate1           -           4.5e-08   30.2   1.1   6.0e-08   29.8   1.1     1       23       5       27       3       28  0.88 zinc finger
"""


def test_parse_pfam_tblout_uses_hmmscan_column_order(tmp_path):
...
def test_parse_pfam_tblout_dedupes_repeated_domain(tmp_path):
    ...
    tblout.write_text(
        "Protein_kinase PF00069.28 candidate1 - 1.2e-30 105.3 0.1 3.4e-31 104.9 0.1 "
        "1 1 1 260 10 265 8 270 0.95 first\n"
        "Protein_kinase PF00069.28 candidate1 - 9.9e-05  20.0 0.1 9.9e-05  20.0 0.1 "
