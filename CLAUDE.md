...
| `--hmm_presence_cov` | `0.5` | Family-HMM min profile coverage *fraction* for presence (`--cluster_tool mmseqs` and `novelty_discovery`); the historical sweep launcher (NovInvenio_Investigations/legacy/novinvenio_scripts/run_param_sweep.sh) included it as a swept dimension (2026-09-03) but no sweep has actually been run against it yet — the shipped 0.5 is still a guess. See `--hmm_presence_min_residues` below and the 2026-09-03 finding that this floor rejects 83% of otherwise-significant hits on HMMs >600aa (expected biology for multi-domain proteins, not promiscuous-domain false positives) |
