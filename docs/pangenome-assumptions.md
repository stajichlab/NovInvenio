...
| `pangenome_polarity_loss_min_frac` | `0.9` | `validated-empirically` | `direction_a_freq` = loss when at least this fraction of outgroup strains carry the family. Replaces, alongside, the strict rule in `direction_a` (loss only if present in **every** outgroup strain), which cannot call loss with a many-strain outgroup: in the pre-rescue Coccidioides reciprocal runs (169 / 360 outgroup strains) it gave 0.29-1.13% loss and 65-72% ambiguous. Calibration: of families present in >=99% of one species' strains, the fraction reaching >=0.90 in the other species is **89.6%** (immitis -> posadasii, n=4,069) and **94.2%** (posadasii -> immitis, n=4,213); only **10.6% / 32.6%** are present in all strains; >=0.95 keeps 87.2% / 92.6%. Present = present or genome_only. | 529-strain Coccidioides rescued matrix (`rescue_structural_genus_vs_ureesii`, 2026-09-24) | An outgroup whose per-strain dropout rate differs a lot (e.g. much more fragmented assemblies) -- the calibration is the dropout rate. | Recompute the ">=99% in species A -> frequency in species B" distribution on the new dataset (NII `notes/pangenome-method-investigations/2026-09-24-rescued-rerun-and-outgroup-polarity.md`). |
| `pangenome_polarity_gain_max_frac` | `0.1` | `unvalidated-assumption` | `direction_a_freq` = gain when at most this fraction of outgroup strains carry the family. Chosen symmetric to `loss_min`; the false-presence rate that should set it has **not been measured**. | **Not yet measured** | A false-presence rate (e.g. paralog/`genome_only` artefacts) well above 10% per strain. | Estimate false-presence per outgroup strain, e.g. from families private to the ingroup in a closely related comparison. |
| **(outgroup divergence)** outgroup presence is only visible at the tier-1/rescue identity cutoffs | — | `validated-empirically` (as a limitation) | With *U. reesii* as outgroup, its proteins match *C. immitis* RS at median **77.4%** identity; only **11.1%** pass >=90% id / >=80% cov (control *C. posadasii* C735: 98.8%, 83.3%). `Uree` is 87.3% singletons and present in 11.7% of core families, so almost every family polarises as gain (138:1). Neither polarity column fixes this; it needs an outgroup within the clustering identity range. | 529-strain Coccidioides `genus_vs_ureesii` | — | Check outgroup-to-ingroup median protein identity against `pangenome_tier1_min_id` before trusting any polarity call. |
