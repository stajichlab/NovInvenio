...
# scores each grid point on #families / #novelties / BUSCO recovery / control recall+FP /
# TBLASTN-removal, and collates one scored table so the shipped default can be picked at
# the knee (bin/collate_sweep.py) rather than guessed.
#
...
#
# Prerequisites:
#   * BUSCO fungi_odb12 full_table.tsv per ingroup proteome (busco -m proteins -l
#     fungi_odb12), listed in BUSCO_TABLES as SHORT=path (curation-free family quality).
...
printf 'min_seq_id\tcov\thmm_evalue\tn_families\tn_novelties\tbusco_recovery\trecall\tfp_rate\ttblastn_removed\n' > "$METRICS"
...
for id in "${MIN_SEQ_IDS[@]}"; do
  for cov in "${COVS[@]}"; do
    for ev in "${HMM_EVALUES[@]}"; do
      ...
      # BUSCO single-copy family recovery (curation-free quality).
      busco_recovery=""
      ...
      printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
        "$id" "$cov" "$ev" "$n_families" "$n_novelties" \
        "$busco_recovery" "$recall" "$fp_rate" "$tblastn_removed" >> "$METRICS"
    done
  done
done
...
echo ">>> wire the recommended default into nextflow.config (family_min_seq_id / family_cov / hmm_presence_evalue)"
